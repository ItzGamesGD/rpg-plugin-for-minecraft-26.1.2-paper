...
- release에서 눈이 급히 열리고, 중앙 눈→플레이어 눈의 3D 조준선과 sonic boom hit ray가 일치하며 정상 속도로 감쇠하는지 확인한다.
...
- marker 원형 위험지역이 작게 유지되고, 본체가 Y 차이를 포함한 실제 좌표까지 각 10 tick 동안 반드시 1→9 순서로 이동하며 이동 방향을 바라보는지 확인한다.
- 9번째 marker 등록 tick에 recording hazard/damage가 한 번 더 실행되지 않는지 확인한다.
- PATH_DASH 완료 후 prototype이 `RECOVERING → IDLE`로 돌아가고 다음 명령이 `true`를 반환하는지 확인한다.
