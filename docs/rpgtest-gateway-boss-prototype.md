...
payloads use their Bukkit vanilla projectile entities for outbound attack/block collision.
A separate following `Interaction` exists only for the slightly enlarged reflection input;
it never deals incoming damage. Reflected routing becomes deterministic only after that
Interaction is attacked. End Crystal Bomb intentionally remains a safe custom ItemDisplay,
with swept block collision, a custom fuse/AoE, and no terrain-changing explosion.
...
a real Trident projectile. Orbital core families occupy coherent tilted rings while both
