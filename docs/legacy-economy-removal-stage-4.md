...
- Farming and alchemy domain mechanics remain without plugin-owned inventory interfaces. Quest access remains available through text commands rather than the removed hub.
...
## Final inventory-UI cleanup

- Plugin-owned crafting, farming hub, delivery, cooking, alchemy, effect-list, stat, and special-equipment inventories have been removed from production.
- `/rpg farming` now explains the world-interaction boundary, `/rpg effect list` reports active effects in chat, and the special-equipment command retains only textual/admin actions.
- Delivery domain state remains available for a future block or entity hand-in interaction, but normal delivery submission is intentionally unavailable until that world-native surface exists.
- Crafting recipes and alchemy runtime definitions remain domain data; their former inventory layout/controller configuration does not.
- Production contains no custom `InventoryHolder` implementation and no `createInventory` or `openInventory` call. Remaining inventory events only observe vanilla containers or refresh physical-item state.
...
4. Verify farming and alchemy mechanics remain usable without opening plugin-owned inventories.
